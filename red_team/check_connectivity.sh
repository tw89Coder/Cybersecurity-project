...
if ping -c 2 -W 2 "$TARGET_IP" > /dev/null 2>&1; then
    echo "  [OK] 攻擊機 → 靶機 ($TARGET_IP) ICMP OK"
else
    echo "  [FAIL] 攻擊機 → 靶機 ($TARGET_IP) ICMP 不通!"
...
if command -v ufw > /dev/null 2>&1; then
    ...
    if echo "$UFW_STATUS" | grep -qi "active"; then
        ...
        if sudo ufw status verbose 2>/dev/null | grep -qi "deny.*icmp"; then
            echo "  [FAIL] UFW 明確封鎖了 ICMP!"
            ERRORS=$((ERRORS + 1))
        fi
    else
        echo "  [OK] UFW 未啟用 (不會封鎖 ICMP)"
    fi
else
    echo "  [OK] UFW 未安裝"
fi
...
if [ "$HTTP_CODE" = "200" ]; then
    echo "  [OK] Flask 靶機在線 (HTTP $HTTP_CODE)"
else
    echo "  [FAIL] Flask 靶機不可達 (HTTP $HTTP_CODE)"
...
if echo "$SSTI_RESULT" | grep -q "49"; then
    echo "  [OK] SSTI 漏洞確認可利用 (7*7=49)"
else
    echo "  [FAIL] SSTI 測試失敗（靶機可能未啟動或漏洞已修補）"
    ERRORS=$((ERRORS + 1))
fi
echo ""
...
if [ "$ARCH" = "x86_64" ]; then
    echo "  [OK] x86_64 — memfd_create syscall #319 正確"
elif [ "$ARCH" = "aarch64" ]; then
    echo "  [OK] aarch64 — memfd_create syscall #279 正確"
else
    echo "  [WARN]  未知架構 $ARCH — memfd_create syscall 號碼需要確認"
fi
...
if [ "$KMAJOR" -gt 3 ] || ([ "$KMAJOR" -eq 3 ] && [ "$KMINOR" -ge 17 ]); then
    echo "  [OK] Kernel $KERNEL 支援 memfd_create (>= 3.17)"
else
    echo "  [FAIL] Kernel $KERNEL 太舊，不支援 memfd_create (需 >= 3.17)"
    ERRORS=$((ERRORS + 1))
fi
echo ""
