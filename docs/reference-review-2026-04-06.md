...
**替代 A — T1 學術會議論文（推薦）**
- **標題**：Malicious ICMP Tunneling: Defense against the Vulnerability
- **作者**：Abhishek Singh, Ola Nordstrom, Chenghuai Lu, et al.
- **會議**：ACISP 2003 (Lecture Notes in Computer Science, vol. 2727, Springer)
- **年份**：2003
- **DOI**：10.1007/3-540-45067-X_20
- **Tier**：T1（同儕審查會議，Springer LNCS）
- **涵蓋內容**：ICMP 不使用 port 多工，封包資料段可供惡意隱蔽通道使用；提出無狀態模型防止 ICMP tunneling，並以 Linux kernel module 實作驗證
- **驗證來源**：Springer Nature Link、ACM DL 交叉列表、Semantic Scholar、ACISP 2003 會議目錄

**替代 B — T1 IEEE 會議論文**
- **標題**：Detection of Covert Channels Over ICMP Protocol
- **作者**：Sirine Sayadi, Tarek Abbes, Adel Bouhoula
- **會議**：2017 IEEE/ACS 14th International Conference on Computer Systems and Applications (AICCSA)
- **年份**：2017
- **DOI**：10.1109/AICCSA.2017.60
- **Tier**：T1（IEEE 會議，同儕審查）
- **涵蓋內容**：監測與偵測 ICMP 隱蔽通道，使用 Ptunnel 工具測試，實驗展示高效偵測能力
- **驗證來源**：HAL 開放檔案庫 (hal-02381398)、IEEE Xplore (document/8308433)

**替代 C — 業界權威來源（MITRE ATT&CK）**
- **標題**：Non-Application Layer Protocol (Technique T1095)
- **作者**：MITRE Corporation
- **來源**：MITRE ATT&CK Enterprise Framework
- **URL**：https://attack.mitre.org/techniques/T1095/
- **涵蓋內容**：明確記錄 APT 組織使用 ICMP 進行 C2 通訊的實例（PingPull、Regin、Cobalt Strike、Winnti for Linux、Uroburos 等）
- **驗證來源**：直接存取 ATT&CK 頁面確認
