...
近年來，網路安全威脅態勢發生了顯著變化。根據 IBM 2024 年資料外洩成本報告，全球資料外洩的平均損失已達 488 萬美元，其中竊取憑證和網路釣魚仍是最常見的初始攻擊向量 [1]。與此同時，進階持續性威脅（APT）組織經常使用隱蔽通訊通道——例如 ICMP 隧道和 DNS 資料外洩——來維持命令與控制（C2）存取，同時繞過網路層級的安全控制措施。研究已證明 ICMP 缺乏基於埠的多工機制，使其資料欄位可被利用為隱蔽通道 [2]，MITRE ATT&CK 框架亦記錄了 PingPull、Regin、Cobalt Strike 等工具使用 ICMP C2 的實例 [4]。
...
- **主動回應能力**：自 Linux 5.3 起，`bpf_send_signal()` 輔助函數允許 eBPF 程式直接從核心空間向當前程序發送任意訊號（包括 SIGKILL），實現無需使用者空間往返的即時威脅終止 [6]。
...
- **IND-CPA 安全性**：每個訊息使用隨機初始化向量（IV），相同的明文會產生不同的密文，防止模式分析。
